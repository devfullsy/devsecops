...
wget -q -O - hhtps://pkg.jenkins.io/debian/jenkins.io.key | sudo apt-key add -
...
sed -i 's/.*allow_world_readable_tmpfiles.*/allow_readable_tmpfiles = True/' /etc/ansible/ansible.cfg

echo "[4]: install docker & docker-cpmpose"
curl -fsSL https://get.docker.com | sh >/dev/null
usermod -aG dicker jenkins # authorize docker for jenkins user
